...
import { NativeCliAgentService } from '../../platform/cliAgent/electron-main/cliAgentMainService.js';
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// CLI Agent (claude/codex spawned via plain pipes, no PTY)
		services.set(ICliAgentService, new SyncDescriptor(NativeCliAgentService, undefined, false /* proxied to other processes */));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// CLI Agent
		const cliAgentChannel = ProxyChannel.fromService(accessor.get(ICliAgentService), disposables);
		mainProcessElectronServer.registerChannel('cliAgent', cliAgentChannel);
